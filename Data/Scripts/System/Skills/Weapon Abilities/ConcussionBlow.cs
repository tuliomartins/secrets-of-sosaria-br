...
namespace Server.Items
{
	...
	public class ConcussionBlow : WeaponAbility
	{
		...
		public override bool OnBeforeDamage( Mobile attacker, Mobile defender )
		{
			...
			int damage = 20; // Base damage is 10.

			if ( defender.HitsMax > 0 ) 
			{
				double hitsPercent = ( (double)defender.Hits / (double)defender.HitsMax ) * 120.0;

				double manaPercent = 0;

				if ( defender.ManaMax > 0 )
					manaPercent = ( (double)defender.Mana / (double)defender.ManaMax ) * 120.0;
