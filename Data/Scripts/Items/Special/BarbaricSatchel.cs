		...
		public class BarbaricAlterGump : Gump
		{
			...
			public BarbaricAlterGump( Mobile from, Item item, string make ): base( 25, 25 )
			{
				...
				else if ( item is BaseHat || ( CraftResources.GetType( item.Resource ) == CraftResourceType.Fabric && item.Layer == Layer.Helm ) )
				{
					...
					AddItem(32, 58, 22088);
					AddItem(78, 59, 0x672E);
					AddItem(136, 60, 5439);
					AddItem(195, 53, 11121);
					AddItem(256, 53, 12648);
					AddItem(322, 58, 0x1718);
					AddItem(384, 63, 0x2798);
			...
			public override void OnResponse( NetState state, RelayInfo info ) 
			{
				...
				else if ( info.ButtonID == 2 ){ 	NewName = "circlet";			itemID = 0x672E; }
				...
				else if ( info.ButtonID == 6 ){ 	NewName = "mask";				itemID = 0x1718; }
				else if ( info.ButtonID == 7 ){ 	NewName = "mask";				itemID = 0x2798; }
